...
const v2 = {};
v2.len      = (a)       => Math.sqrt(a[0] * a[0] + a[1] * a[1]);               // the magnitude of the vector.
...
v2.rad      = (a)       => Math.atan2(a[1], a[0]);                             // the radian angle counter-clockwise from x-axis, from -pi to pi.
v2.from_rad = (r)       => [ Math.cos(r),     Math.sin(r)    ];                   // the unit vector converted from the radian angle; inverse of rad().
...
v2.rad_to_deg   = (r)   => r * 180 / Math.PI;
v2.deg_to_rad   = (d)   => d * Math.PI / 180;
v2.unit_deg     = (d)   => d % 360;                                         // normalize degree to within -360 to 360

const v3 = {};
v3.len      = (a)       => Math.sqrt(a[0] * a[0] + a[1] * a[1] + a[2] * a[2]); // the magnitude of the vector.
